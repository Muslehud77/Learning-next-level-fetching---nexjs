...
const PostsPage = async () => {
    ...
    return (
        <div className='container mx-auto'>
            <h1 className='text-5xl text-center mt-5'>Posts SSG Static Site Generation  : {posts.length}</h1>
                    ...
                    posts.map((post) =><div key={post.id} className="card w-96 bg-base-100 shadow-xl">
                      <div className="card-body">
                        ...
                        <div className="card-actions justify-end">
                          <Link href={`/posts-SSR/${post.id}`} className="btn btn-primary">Read More</Link>
                        </div>
                        </div>
                      </div>)
                }
